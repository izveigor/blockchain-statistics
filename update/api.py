...
import time 
from blockchain.constants import URL, NEW_BLOCK_WAITING_TIME, NEED_FIELDS_OF_INPUTS_AND_OUT, NEED_FIELDS_OF_TRANSACTION, NEED_FIELDS_OF_BLOCK
import json
import asyncio
import websockets
...
def _get_data_from_transactions(transactions):
    ...
    for transaction in transactions:
        ...
        for input_ in tx_result.pop('inputs'):
            ...
            if not (Transaction.objects.filter(tx_index=prev_tx_index).exists() or previous_tx_index_in_tx_indexes):
        ...
        if data_from_transactions['max_price_of_transaction']['price'] < tx_result['price']:
            data_from_transactions['max_price_of_transaction'].update({**tx_result})

        if data_from_transactions['number_of_inputs']['number_of_inputs'] < tx_result['number_of_inputs']:
            data_from_transactions['number_of_inputs'].update({**tx_result})

        if data_from_transactions['number_of_outputs']['number_of_outputs'] < tx_result['number_of_outputs']:
            data_from_transactions['number_of_outputs'].update({**tx_result})
    
    Transaction.objects.bulk_create([Transaction(tx_index=tx_index) for tx_index in tx_indexes])
