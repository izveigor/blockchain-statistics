...
from update.api import _get_data_from_transactions, get_block_api, get_latest_block_height
...
class TestAPI(UnitTest):
    ...
    def test_block_data(self, mock_blockchain_update, mock_get):
        ...
        mock_blockchain_update.assert_called_once_with(first_block_result, JsonData.first_data_from_transactions)
...
class TestGetDataFromTransactions(UnitTest):
    ...
    def test_get(self):
        ...
        for transaction_model, transaction in zip(Transaction.objects.all(), reversed(sorted(transactions, key=lambda x:x['tx_index']))):
            self.assertEqual(
                transaction_model.tx_index,
                transaction['tx_index']
            )
    
    def test_get_if_a_previous_transaction_in_the_database(self):
        transactions = JsonData.first_block['tx']
        Transaction.objects.create(tx_index=transactions[0]['inputs'][0]['prev_out']['tx_index'])
        ...
        test_data_from_transactions['number_of_satoshi'] -= transactions[0]['inputs'][0]['prev_out']['value']

        self.assertEqual(
            data_from_transactions,
            test_data_from_transactions
        )
    
    def test_get_if_a_previous_transaction_in_tx_indexes(self):
        transactions = JsonData.first_block['tx']
        delta = transactions[1]['inputs'][0]['prev_out']['value']
        transactions[1]['inputs'][0]['prev_out']['tx_index'] = transactions[0]['tx_index']
    ...
    def test_if_inputs_addr_equals_outputs_addr(self):
        ...
        transactions[0]['out'][1]['addr'] = transactions[0]['inputs'][0]['prev_out']['addr']
