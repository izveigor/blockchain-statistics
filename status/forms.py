...
class TimelineBlockchainForm(forms.Form):
    ...
    '''
    date_start = forms.CharField(widget=forms.TextInput(attrs={'type': 'date', 'class': 'form-control', 'id': 'form_date_start'}))
    time_start = forms.CharField(widget=forms.TextInput(attrs={'type': 'time', 'class': 'form-control', 'id': 'form_time_start', "step": 1}))
    date_end = forms.CharField(widget=forms.TextInput(attrs={'type': 'date', 'class': 'form-control', 'id': 'form_date_end'}))
    time_end = forms.CharField(widget=forms.TextInput(attrs={'type': 'time', 'class': 'form-control', 'id': 'form_time_end', "step": 1}))
    ...
    def clean(self):
        ...
        start = int(time.mktime(
            datetime.strptime(
                cleaned_data['date_start'].replace('-', ' ') + ' ' + cleaned_data['time_start'].replace(':', ' '), 
                '%Y %m %d %H %M %S'
            ).timetuple()
        ))
        end = int(time.mktime(
            datetime.strptime(
                cleaned_data['date_end'].replace('-', ' ') + ' ' + cleaned_data['time_end'].replace(':', ' '), 
                '%Y %m %d %H %M %S'
            ).timetuple()
        ))
